...
The \texttt{Subtraction} class is little more than an interface, with only the constructors and destructors not being pure virtual. However, the \texttt{Estimation} class needs to keep track of the current noise estimation (in the form of an array of \texttt{doubles}). For this, I choose to use the \textbf{Template} design pattern:
